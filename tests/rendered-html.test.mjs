...
test("server-renders the Semana do Plástico landing page", async () => {
  ...
  assert.match(html, /googletagmanager\.com\/ns\.html\?id=GTM-NHB94476/);
  assert.ok((html.match(/GTM-NHB94476/g) ?? []).length >= 2);
  assert.ok(html.indexOf("GTM-NHB94476") < html.indexOf('<meta charset="utf-8"'));
  assert.match(
    html,
    /<body>\s*<!-- Google Tag Manager \(noscript\) -->\s*<noscript>/,
  );
  assert.doesNotMatch(
    html,
...
test("keeps the event content centralized and the page in full-page scroll mode", async () => {
  const [page, layout, styles, appStyles, reveal, workerSource] = await Promise.all([
  ...
  assert.match(layout, /GTM-NHB94476/);
